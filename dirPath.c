				...
				)
{
  ...
  if(argc > 1)
  ...
  else
    {
      ...
      textSpace[strlen(textSpace) - 1] = 0;
      return(textSpace);
    }
				...
				)
{
  ...
  dirEntryName->name_ = (char*)malloc((linePtr-entryNamePtr)+ 1);
  strncpy(dirEntryName->name_ , entryNamePtr, (linePtr-entryNamePtr) + '\0');
...
{
  //  YOUR CODE HERE
  struct DirEntryName* crntDirName;
  struct DirEntryName* nxtDirName;
  char* entryName;

  crntDirName = pathNamePtr->dirEntryNamePtr_;
  free(pathNamePtr);

  while(crntDirName != NULL)
  {
    entryName = crntDirName->name_;
    free(entryName);
    nxtDirName = crntDirName->nextPtr_;
    free(crntDirName);
    crntDirName = nxtDirName;
  }
}
